...
struct MatmulToLLVMConverter : public OpConversionPattern<linalg::MatmulOp> {
  ...
  matchAndRewrite(linalg::MatmulOp op, OpAdaptor adaptor,
    ...
    // Define vector sizes for ARM NEON and SVE
    int neonVectorSize = 4; // 4 floats for float32x4_t in NEON
    int sveVectorSize = 16; // example size for SVE, it's scalable so actual size may vary

    int M_dim = A.getType().cast<MemRefType>().getDimSize(0);
    int N_dim = B.getType().cast<MemRefType>().getDimSize(1);
    int K_dim = A.getType().cast<MemRefType>().getDimSize(1);

    // Choose vector size based on matrix size (for demonstration)
    int vectorSize = (K_dim % sveVectorSize == 0) ? sveVectorSize : neonVectorSize;

    // Outer loops
    auto loop1 = rewriter.create<scf::ForOp>(loc, 0, M_dim, 1, ValueRange{}, [&](OpBuilder &b1, Location loc, Value i, ValueRange args1) {
      auto loop2 = b1.create<scf::ForOp>(loc, 0, N_dim, 1, ValueRange{}, [&](OpBuilder &b2, Location loc, Value j, ValueRange args2) {
        
        Value sum = b2.create<arith::ConstantFloatOp>(loc, llvm::APFloat(0.0), rewriter.getF32Type());

        // Vectorized loop
        auto loop3 = b2.create<scf::ForOp>(loc, 0, K_dim - (K_dim % vectorSize), vectorSize, ValueRange{sum}, [&](OpBuilder &b3, Location loc, Value k, ValueRange args3) {
          // Load multiple elements into a vector register
          Value aVec = b3.create<LLVM::MaskedLoadOp>(loc, VectorType::get(vectorSize, rewriter.getF32Type()), A, ValueRange{i, k}, /*mask=*/nullptr, /*alignment=*/0);
          Value bVec = b3.create<LLVM::MaskedLoadOp>(loc, VectorType::get(vectorSize, rewriter.getF32Type()), B, ValueRange{k, j}, /*mask=*/nullptr, /*alignment=*/0);

          // Vectorized multiplication and addition
          Value productVec = b3.create<LLVM::FMulOp>(loc, aVec, bVec);
          Value newSum = b3.create<LLVM::FAddOp>(loc, args3[0], productVec);

          b3.create<scf::YieldOp>(loc, newSum);
        });

        // Tail loop
        auto tailLoop = b2.create<scf::ForOp>(loc, K_dim - (K_dim % vectorSize), K_dim, 1, ValueRange{loop3.results()[0]}, [&](OpBuilder &b3, Location loc, Value k, ValueRange args3) {
        ...
        b2.create<memref::StoreOp>(loc, tailLoop.results()[0], C, ValueRange{i, j});
      });
    });
