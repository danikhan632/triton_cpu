...
int main()
{
    uint8_t input_left[M*K];
    uint8_t input_right[K*N];

    uint32_t output_ref[M*N] = {0};
    uint32_t output_opt[M*N] = {0};

    srand((unsigned int)time(0));

    for(int64_t x = 0; x < (M * K); ++x)
    {
        input_left[x] = (uint8_t)(rand() % 256);
    }
    for(int64_t x = 0; x < (K * N); ++x)
    {
        input_right[x] = (uint8_t)(rand() % 256);
    }
